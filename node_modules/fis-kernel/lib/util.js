...
_.toEncoding = function(str, encoding){
    return getIconv().toEncoding(str + '', encoding);
};
...
_.ext = function(str){
    ...
    if((pos = str.lastIndexOf('/')) > -1){
        info.dirname = pos === 0 ? '/' : str.substring(0, pos);
...
_.request = function(url, opt, callback){
    ...
    var http = /^https:/i.test(url) ? require('https') : require('http'),
        ...
        tmp  = fis.project.getTempPath('download', name),
        data = opt.data;
    delete opt.data;
    _.write(tmp, '');
            ...
            res
                ...
                .on('end', function(){
                    if(status >= 200 && status < 300 || status === 304){
                        if(callback){
                            if(typeof callback(null, tmp, res) === 'undefined'){
                                fs.unlinkSync(tmp);
                            }
                        }
                    } else {
...
_.readJSON = function(path){
    ...
    try {
        ...
        fis.log.error('parse json file[' + path + '] fail.');
    }
    return result;
};
