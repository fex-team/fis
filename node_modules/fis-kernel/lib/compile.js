...
function process(file){
    ...
    if(file.isLib || file.isModule){
        pipe(file, 'modular', file.rExt);
    }
    standard(file);
    if(exports.settings.hint){
        pipe(file, 'hint', file.rExt, true);
    }
...
function embeddedCheck(main, embedded){
    ...
    if(main === embedded){
        ...
        var next = embeddedMap[embedded],
            msg = [embedded];
        while(next !== embedded){
            msg.push(next = embeddedMap[next]);
        }
        msg.push(embedded);
        ...
        content = content.replace(exports.lang.reg, function(all, type, value){
            ...
            switch(type){
                ...
                case 'uri':
                    info = fis.uri.getUrl(
                        value, file.dirname,
                        exports.settings.hash,
                        exports.settings.domain
                    );
                    ret = info.quote + info.url + info.query + info.quote;
                    break;
                case 'dep':
                    if(file.cache){
                        info = fis.uri(value, file.dirname);
                        if(info.file){
                            file.cache.addDeps(info.file);
                        } else if(fis.util.isAbsolute(info.rest)) {
                            file.cache.addDeps(info.rest);
                        }
                    }
                    break;
                case 'embed':
